...
# [3.1 — item 2] Segunda subnet para isolar a futura CAMADA DE APLICAÇÃO da QC.
# Fica na mesma VNet (10.0.0.0/16), em um range distinto da subnet "default".
# Por ora não tem recurso anexado — é a fundação para separar app de gestão/SSH
# nas próximas aulas (ex.: App Service / Functions / Container Apps).
...
resource "azurerm_network_security_group" "nsg" {
  ...
  # [3.1 — item 1] SSH não fica mais aberto para o mundo (`*`).
  # Liberamos a porta 22 APENAS para o seu IP público (/32), via variável `meu_ip`.
  # No Cloud Shell: meu_ip = "$(curl -s ifconfig.me)".
  security_rule {
    name                       = "SSH"
    priority                   = 300
    direction                  = "Inbound"
